...
void recursiveRangeQuery(k2_tree<2>& G, int v, int x, int y, int x0, int y0, int s, int range, vector<vector<long long> > &R){
  if(s == 1) {
    R.push_back(vector<long long>({x0+1,y0+1,max(abs(x-x0-1),abs(y-y0-1))}));
  }
  else{
...
void top(long long xdist, long long ydist, int v, int l, int x0, int y0, int x, int y, int s, k2_tree<2>& G, priority_queue<vector<long long>, vector<vector<long long> >, compare2> &R, long long m){
  if (R.size() >= m && (m==0 || max(xdist,ydist) >= R.top()[2])){
    return;
  }
  if (s==1){
    if (R.size() == m){
    ...
    R.push(vector<long long>({x0+1,y0+1,max(xdist,ydist)}));
  }
  else if (s!=1){
    int new_v, new_x0, new_y0;
    long long new_xdist, new_ydist;
    int k = G.get_k();
    int len = s/k;
    int r1 = 1;
    int c1 = 1;
    ...
    for (int r = r1; r<=r2; r++){
      for (int c = c1; c<=c2; c++){
        if (childrenlabeled(G, v, (r-1)*k+c)){
          ...
          top(new_xdist, new_ydist, new_v, l+1, new_x0, new_y0, x, y, len, G, R, m);
        }
      }
    }
  }
