...
vector<pair<int, int> > closest1(k2_tree<2>& G, int x, int y, int m){
  ...
  Q.push(vector<long long>({1,1,0,0,0,G.get_size()}));
      ...
      for (int r = r1; r<=r2; r++){
        for (int c = c1; c<=c2; c++){
          ...
          if (childrenlabeled(G, v, (r-1)*k+c)){
            ...
            //cout << "pusheare" << endl;
            Q.push(vector<long long>({new_v, l+1, new_d, new_x0, new_y0, len}));
            //cout << "pushie" << endl;
          }
        }
      }
...
vector<pair<int, int> > closest3(k2_tree<2>& G, int x, int y, int m){
  ...
  if (range<maxR && Mdistance(ret, m) > range){
    ret = rangeQuery(G, x, y, Mdistance(ret, m));
  }
  return getM(ret, m);
}
